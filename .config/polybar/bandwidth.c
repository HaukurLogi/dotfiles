/* Adapted from bandwidth2 at https://github.com/vivien/i3blocks-contrib
 * This program is free software, licensed under the UNLICENSE. */
             ...
             double b, int const warning, int const critical)
{
  ...
  if (b < divisor) {
    ...
    printf("%.1lf", b / divisor);
    printf(" K%c/s", unit);
  } else if (b < divisor * divisor * divisor) {
    printf("%.1lf", b / (divisor * divisor));
    printf(" M%c/s", unit);
  } else {
    printf("%.1lf", b / (divisor * divisor * divisor));
    printf(" G%c/s", unit);
  }
  printf("\%{F-}");
}
